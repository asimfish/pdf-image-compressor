...
function app() {
  return {
    ...
    _validTargetSize(v) {
      ...
      if (!unit && bytes < 1000) return 'Bare numbers under 1000 need a unit (e.g. 500KB, 2MB)';
      return '';
    },
    _validateForm(form) {
          ...
          if (e.lengthComputable) {
            ...
            if (filePct >= 1 && !processingTimer) {
              ...
              processingTimer = setInterval(() => {
                ...
                this.uploadStatus = `Compressing ${index + 1}/${total}: ${file.name} ⏳ ${elapsed}s`;
              }, 1000);
            }
          }
        };
    ...
    compressPdf(pdf) {
      ...
      const pct = base ? Math.round(base * 0.8) : 0;
      const target = pct >= 1000 ? Math.min(base, pct) : null;
      this.compressForm = this._defaults({ target_size: target ? this.fmtSize(target) : '', label: '' });
      this._resetDragState();
